...
# define truncated studentt function
    ...
    for (j in 1:num_steps){ # s_ad
      ...
      for (k in 1:num_steps){ # b
        ...
        for (n in 1:num_runs_each){ # n
          ...
          r_realized[h,i,j,k,n,1] <- rtrunct(1, r_mean, sigma_r)
          ...
          b_realized[h,i,j,k,n,1] <- rtrunct(1, b_mean, sigma_b)
          ...
          for (yr in 2:num_years_sim){
            ...
            r_realized[h,i,j,k, n,yr] <- rtrunct(1, r_mean, sigma_r)
            ...
            # juvenile survival
            s_juv_realized[h,i,j,k, n,yr] <- rtrunct(1, s_juv_mean, sigma_s_juv)
            s_juv <- s_juv_realized[h,i,j,k, n,yr]
            # adult survival
            s_ad_realized[h,i,j,k, n,yr] <- rtrunct(1, s_ad_mean, sigma_s_ad)
            ...
            # breeding probability
            b_realized[h,i,j,k, n,yr] <- rtrunct(1, b_mean, sigma_b)
...
save(B_total, N_adults_total, r_mean, s_juv_mean, s_ad_mean, b_mean, r_realized, s_juv_realized, s_ad_realized, b_realized, file = "Agestructured_simulations_studenttdemography_data.RData")
